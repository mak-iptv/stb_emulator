\
    ...
    export default function App() {
      ...
      const [portalName] = useState('STB Portal Demo');
      ...
      useEffect(() => {
        ...
        setFilteredChannels(channels.filter(c => (c.name || '').toLowerCase().includes(s) || (c.group||'').toLowerCase().includes(s)));
      }, [search, channels]);
      ...
      async function loginSubmit(e) {
        ...
        // Try Xtream/XUI player_api directly (username+password)
        if (base && login && login.id && password) {
          ...
          try {
            ...
            console.warn('Direct player_api fetch failed (CORS or network):', err);
          }

          // Try via proxy if direct failed
          if (!loaded) {
            try {
              const proxyResp = await fetch(`/mag-proxy?target=${encodeURIComponent(apiUrl)}`);
              if (proxyResp.ok) {
              ...
              console.warn('Proxy player_api failed:', err);
            }
          }
        }

        // If not loaded, try MAG/STALKER via proxy (MAC)
        if (!loaded && base && login && login.id) {
          try {
            const mac = login.id;
            const hsUrl = `${base}/stalker_portal/server/load.php?type=stb&action=handshake&JsHttpRequest=1-xml&mac=${mac}`;
            const hs = await fetch(`/mag-proxy?target=${encodeURIComponent(hsUrl)}`);
            if (hs.ok) {
              const getAll = `${base}/stalker_portal/server/load.php?type=itv&action=get_all_channels&JsHttpRequest=1-xml`;
              const chRes = await fetch(`/mag-proxy?target=${encodeURIComponent(getAll)}`);
              if (chRes.ok) {
                const text = await chRes.text();
                try {
                  const obj = JSON.parse(text);
                  const list = obj.js && obj.js.data ? obj.js.data : obj.data || obj;
                  if (Array.isArray(list)) {
                    const chans = list.map(ch => ({
                      name: ch.name || ch.title,
                      url: ch.cmd || ch.stream_url || '',
                      logo: ch.icon || null,
                      group: ch.category_name || ch.group || 'Live'
                    }));
                    if (chans.length) {
                      setChannels(chans);
                      setFilteredChannels(chans);
                  ...
                  console.warn('Failed parse STALKER channels JSON', parseErr);
                }
              }
            }
          } catch (err) {
            console.warn('MAG/STALKER via proxy failed', err);
          }
        }
                    ...
                    <div key={group} className="mb-3">
                      ...
                      {chans.map((ch) => (
                        <div
                          ...
                          className={`p-2 rounded cursor-pointer ${currentIdx === filteredChannels.indexOf(ch) ? 'bg-blue-600' : 'hover:bg-gray-700'}`}
                        >
                          {ch.name}
                        </div>
                      ))}
